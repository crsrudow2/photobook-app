...
**Branch:** feat/photobook-app-storefront

## Description

Build the storefront UI for the photo book app. Tasks: 1) Create src/app/(shop)/page.tsx — homepage with hero section, product category grid, featured products. 2) Create src/app/(shop)/products/page.tsx — product listing page with filters (size, cover type, price). 3) Create src/app/(shop)/products/[slug]/page.tsx — full PDP with product images, size/cover selector, price, Add to Cart button, breadcrumbs, reviews section. 4) Create src/components/ui/ — Button, Card, Badge, Select components (Tailwind only, no external UI lib). 5) Create src/components/layout/Header.tsx (nav, cart icon, user menu) and Footer.tsx. 6) Create src/app/(shop)/layout.tsx with header/footer. 7) Style everything as a premium photo products brand with purple accent #512dcb. Commit all.
